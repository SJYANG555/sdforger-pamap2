...
submit_variant() {
  local variant="$1"
  local config_path="config/pamap2_sdforger_gemma2_2b_2act_${variant}_ep10.yaml"
  local dataset_dir="artifacts/pamap2_sdforger_dataset_2act_${variant}"
  local checkpoint_dir="outputs/checkpoints/gemma_2act_${variant}_ep10"
  local generated_dir="outputs/generated/gemma_2act_${variant}_ep10"
  local similarity_dir="outputs/evaluation/similarity/gemma_2act_${variant}_ep10"
  local utility_dir="outputs/evaluation/utility/gemma_2act_${variant}_ep10"
  local plots_dir="outputs/plots/gemma_2act_${variant}_ep10"

  local train_job
  train_job="$(sbatch --parsable \
    --job-name="gemma_${variant}_ep10_train" \
  ...
  generate_job="$(sbatch --parsable \
    ...
    --job-name="gemma_${variant}_ep10_gen" \
  ...
  evaluate_job="$(sbatch --parsable \
    ...
    --job-name="gemma_${variant}_ep10_eval" \
